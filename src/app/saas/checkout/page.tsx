...
function ScaleCraftSaaSCheckoutContent() {
  ...
  const productIdParam = searchParams?.get('productId') || searchParams?.get('product') || 'scalecraft-agent-saas';
  const [productId, setProductId] = useState(productIdParam);
  const [productName, setProductName] = useState('ScaleCraft Agent - Managed SaaS');
  const [price, setPrice] = useState<number>(7499); // SaaS Setup price
  const [monthlyPrice, setMonthlyPrice] = useState<number>(800);
  const [loadingPrice, setLoadingPrice] = useState<boolean>(true);

  // SaaS specific states
  const [businessName, setBusinessName] = useState('');
  const [businessType, setBusinessType] = useState('local_services');
  ...
  useEffect(() => {
    const fetchProductPrice = async () => {
      try {
        const targetId = productIdParam;
        setProductId(targetId);
        
        const { data: product } = await supabase
          .from('saas_products')
          .select('id, name, price, test_mode, setup_price, monthly_price')
          .or(`id.eq.${targetId},slug.eq.${targetId}`)
          .maybeSingle();

        if (product) {
          setProductName(product.name || 'ScaleCraft Agent - Managed SaaS');
          ...
          if (isTrial) {
            ...
            setPrice(Number(cleanSetup) || 6499);
    ...
    fetchProductPrice();
  }, [searchParams, isTrial, productIdParam]);
  ...
  useEffect(() => {
    ...
    const savedBizType = localStorage.getItem('buyer_business_type') || 'local_services';
    ...
    setTimeout(() => {
      ...
      if (savedBizType) setBusinessType(savedBizType);
  ...
  const handleApplyCoupon = async (e: React.MouseEvent) => {
    ...
    try {
      const targetId = 'scalecraft-agent-saas';
  ...
  useEffect(() => {
    if (searchParams?.get('trial_used') === 'true') {
      setError('You have already used your 7-day trial. We have automatically switched you to the standard plan.');
    }
  }, [searchParams]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. First record legal agreement and IP address on the backend
      ...
      if (!saveTermsRes.ok) {
        if (resData.error === 'already_had_trial') {
          ...
          setTimeout(() => {
            window.location.href = `/saas/checkout?trial_used=true`;
          }, 3000);
          return;
        }
      ...
      const registeredClientId = resData.clientId;
      ...
      localStorage.setItem('buyer_business_type', businessType);
      ...
      // 3. Trigger Payment Gateway checkout
      await initiateCheckout({
        amount: finalPrice,
        currency: 'INR',
        name: 'ScaleCraft',
        description: isTrial ? `${productName} - 7-Day Trial Activation` : `${productName}`,
        productId: productId,
        buyerName: name.trim(),
        ...
        <div className="md:col-span-5 bg-[#F7F7F5] border-r border-[#EBEBEB] p-8 sm:p-10 flex flex-col justify-between relative">
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <span className="inline-block bg-[#ecfdf5] border border-[#a7f3d0] text-[#047857] text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full">
                {isTrial ? '7-Day Free Trial' : 'Fully Managed (SaaS)'}
              </span>
              <h3 className="font-heading text-xl font-extrabold tracking-tight text-[#111110]">
                {isTrial ? `${productName} - 7-Day Trial` : productName}
              </h3>
              <p className="text-xs text-[#6f6e69] leading-relaxed">
                {isTrial
                  ? `Activate your 7-day ${productName} trial. Includes VPS server setup, same as paid.`
          ...
          <div className="pt-8 border-t border-[#EBEBEB] mt-8 space-y-3">
            ...
            <p className="text-[10px] text-[#aeaca5] mt-1">
              {isTrial
                ? `After 7 days: ₹6,499 setup + ₹${monthlyPrice.toLocaleString('en-IN')}/month`
        ...
        <div className="md:col-span-7 p-8 sm:p-10 flex flex-col justify-center relative z-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <label htmlFor="checkout-biz-type" className="text-xs font-bold uppercase tracking-wider text-[#6f6e69] ml-1">Business Category</label>
                {/* 5 core categories. To add more: add to CATEGORY_MAP, add keywords to analyze_leads.py, add UI badge and filter */}
                <select
                  id="checkout-biz-type"
                  value={businessType}
                  onChange={(e) => setBusinessType(e.target.value)}
                  className="w-full bg-[#F7F7F5] border border-[#EBEBEB] rounded-xl px-4 py-4 text-[#111110] focus:outline-none focus:bg-white focus:border-[#0055ff]/50 focus:ring-1 focus:ring-[#0055ff]/50 transition-all text-sm font-semibold cursor-pointer"
                  required
                >
                  <option value="local_services">Local Services (Restaurant, Salon, Gym, Clinic, etc.)</option>
                  <option value="ecommerce">Ecommerce & Retail (Products, Shop, Online Store)</option>
                  <option value="real_estate">Real Estate & Property</option>
                  <option value="professional">Professional Services (IT, Education, Consulting, Travel)</option>
                  <option value="scalecraft">Digital Products & SaaS (ScaleCraft Platform)</option>
                </select>
              </div>
...
export default function ScaleCraftSaaSCheckout() {
  return (
    <Suspense fallback={
